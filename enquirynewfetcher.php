...
if ($_SESSION["status"] == 1){    
        $getnewEnquiries = "SELECT * from enquiry join tenant on enquiry.tenant_ID = tenant.User_ID  join landlord on landlord.User_ID = enquiry.landlord_ID  where enquiry.tenant_ID = ". $_SESSION['accountID']." AND enquiry.Sender_ID = ". $_SESSION['accountID']." AND status = 1 group by enquiry.EnquiryNumber ORDER BY LastUpdate DESC LIMIT 10 OFFSET $offset";}
    else{
        $getnewEnquiries = "SELECT * from enquiry join tenant on enquiry.tenant_ID = tenant.User_ID  join landlord on landlord.User_ID = enquiry.landlord_ID  where enquiry.landlord_ID = ". $_SESSION['accountID']." AND enquiry.Sender_ID <> ". $_SESSION['accountID']." AND status = 1 group by enquiry.EnquiryNumber ORDER BY LastUpdate DESC LIMIT 10 OFFSET $offset";
...
<div class="enquiry-row" id="row-<?php echo $row['EnquiryNumber']?>">
    ...
    <div class="title-cover">
                ...
                <div class="enquiry-row-header-Name"><?php 
        if ($_SESSION["status"] == 1){    
    ...
    <div class="enquiry-row-header-date"><?php echo date_format($date, 'h:ia - d/m/y');  ?></div>
                <div class="enquiry-row-header-actions">
                    ...
                    <div class="actions-close action-button" id="close-<?php echo $row['EnquiryNumber']?>">Close <i class="fa fa-times" aria-hidden="true"></i></div>
                    <div class="actions-reply action-button" id="reply-<?php echo $row['EnquiryNumber']?>">Reply <i class="fa fa-reply" aria-hidden="true"></i></div>
